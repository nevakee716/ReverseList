...
(function(cwApi, $) {
  ...
  cwLayoutReverseList.prototype.setOriginalSchema = function() {
    ...
    //this.viewSchema = JSON.parse(JSON.stringify(this.backupSchema));
    //cwApi.ViewSchemaManager.setSchema(view, this.backupSchema);

    for (var nodeID in this.backupSchema.NodesByID) {
      if (this.backupSchema.NodesByID.hasOwnProperty(nodeID)) {
        let n = this.backupSchema.NodesByID[nodeID];
        this.viewSchema.NodesByID[nodeID] = n;
      }
    }
  };
  ...
  cwLayoutReverseList.prototype.drawAssociations = function(output, associationTitleText, object) {
    ...
    children = [];
    var viewSchema = this.viewSchema;
  ...
  cwLayoutReverseList.prototype.reverseSchema = function(viewSchema, childId, fatherId) {
    ...
    if (!hasChildrenNodes) {
      ...
      // set replace layout
      viewSchema.NodesByID[this.nodeID].LayoutName = this.options.CustomOptions["replace-layout"];
      viewSchema.NodesByID[this.nodeID].LayoutDrawOne = this.options.CustomOptions["replace-layout"];
    }
