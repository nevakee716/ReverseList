...
(function (cwApi, $) {
    ...
    cwApi.ViewSchemaManager = (function () {
        var that = {}, setSchema,
            getFirstRootNodeSchemaForCurrentView, getFirstRootNodeSchemaForView, getNodeSchemaById, getNode, getPageSchema, getCurrentViewSchema, pageExists, getAllPropertiesGroupFromANode, getPropertyGroup, getNodeSchemaByIdForCurrentView, getPropertyGroupForCurrentView;
        that.schemas = {};
        ...
        function loadSchema(callback) {
            if (cwApi.isModelSelectionPage() === false) {
                var url;
                url = cwApi.getSiteMediaPath() + 'metamodel/schema.' + cwApi.getSelectedLanguage() + '.' + cwApi.getJSONExtention() + '?' + cwApi.getDeployNumber();
